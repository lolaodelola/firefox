...
// Helper to read the current icon URL from the switcher element.
// listStyleImage returns url("chrome://..."), then strip.
function getSwitcherIconUrl(win) {
  let el = win.gURLBar.querySelector(".searchmode-switcher-icon");
  let val = el.style.listStyleImage;
  // val is either "" (null icon) or url("chrome://...")
  if (!val) {
    return null;
  }
  // Strip url(" and trailing ")
...
// Since we are setting icon as null in updateSearchIcon, don't actually check for magnifying glass icon.
